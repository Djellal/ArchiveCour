...
{
    public partial class EditCourrierComponent : ComponentBase
    {
        ...
        {
            var appdbGetCourrierBycouridResult = await Appdb.GetCourrierBycourid(courid);
            courrier = appdbGetCourrierBycouridResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(ArchiveCorr.Models.Appdb.Courrier args)
        {
            try
            {
                var appdbUpdateCourrierResult = await Appdb.UpdateCourrier(courid, courrier);
                DialogService.Close(courrier);
            }
            catch (System.Exception appdbUpdateCourrierException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Courrier" });
            }
        }
